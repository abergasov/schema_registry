...
func TestRegistry_UserStream(t *testing.T) {
	testUser := &user.UserStreamV1{PublicID: uuid.New().String()}
...
func TestRegistry_TaskStream(t *testing.T) {
	taskV1 := &task.TaskStreamV1{PublicID: uuid.New().String()}
	taskV2 := &task.TaskStreamV2{PublicID: uuid.New().String()}
	...
	tsk1, ok := msgMapV1[1].(*task.TaskStreamV1)
	assert.Equal(t, ok, true)
	...
	tsk2, ok := msgMapV2[2].(*task.TaskStreamV2)
	assert.Equal(t, ok, true)
